...
// Gets value stored at key and returns value as a string
//WORKING
std::string handler::dht_get_entry_contents(std::string key) {
...
//WORKING
void handler::hivefs_dht_init_node() {
...
//WORKING
std::string handler::rand_str( size_t length )
{
    auto randchar = []() -> char
    {
        const char charset[] =
...
//WORKING
std::string handler::gen_uid(bool is_dir) {
...
size_t handler::find_attr_index(const std::string& attrs, int prev_delim) {
  size_t pos = 0;
  int count = 0;
...
//WORKING
int handler::hivefs_dht_mkdir(std::string path) { //Is passed path from hive_fuse_mkdir(path)
...
int handler::hivefs_dht_unlink(std::string path) {
    ...
    for (int i = 0; i < d.entries_in_path.size(); ++i) { new_value += d.entries_in_path.at(i); }
    d.value = new_value;
...
//TO FIX
int handler::hivefs_dht_rmdir(std::string path) {
  ...
  for(int i = 0; i < old_d.entries_in_path.size(); ++i) {
...
//WORKING
int handler::hivefs_dht_create(std::string path) {
    if(path.substr(path.length() - 1) == "/") { return -1; }
...
//TO FIX
int handler::hivefs_dht_rename(std::string path, std::string newpath){
  std::string value = dht_get_entry_contents(path);
...
//TO FIX
std::string handler::hivefs_dht_read(std::string path) {
    std::string path_kvs = dht_get_entry_contents(path);
    if(path[path.length() - 1] == '/') { return "-1"; }
...
//TO FIX
int handler::hivefs_dht_write(std::string path, std::string data) {
  std::string value = dht_get_entry_contents(path);
  if(path[path.length() - 1] == '/') { return -1; }
  if(value == "" || (value[value.length() - 1] == '0')) { return -1; } // if doesn't exist, return -1
...
//TO FIX
int handler::hivefs_dht_getattr(std::string path, struct stat& statbuf) {
    std::string path_kvs = dht_get_entry_contents(path);
...
//TO FIX
std::vector<std::string> handler::hivefs_dht_readdir(std::string path) {
